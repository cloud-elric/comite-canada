...
class JuecesAdminController extends Controller {
	...
	public function actionIndex() {
		$porcentajeCategoria = ViewPorcentajeCategoria::model ()->findAll ( array("condition"=>"id_contest=6", "order"=>"txt_name") );
	...
	public function actionJudgeProgress() {
		...
		$relJueces = JueRelJuecesContests::model()->findAll("id_contests=6");
		$juez = [];
		$i = 0;
		foreach($relJueces as $j){
			$juez[$i] = $j->id_juez;
			$i++;
		}
		$criteria = new CDbCriteria();
		$criteria->condition = "b_juez_admin=0";
		/*$criteria->params = array (
			":idJuez" => $idJuez,
			":idContest" => $concurso->id_contest,
		);*/
		$criteria->addInCondition("id_juez", $juez);
	...
	public function actionFinalists($places = 3) {
		...
		$picConflictos = ViewPicsCalificadas::model ()->findAll ( "b_calificada=1 AND (b_conflicto_mencion=1 OR b_conflicto_categoria=1) AND id_contest=6" );
		
		$categoriasCal = ViewPorcentajeCategoria::model()->findAll('id_contest=6');
		$total = 0;
		...
		->where('id_contest = 6 AND b_empate=1 AND b_calificada_desempate=0')
		->queryAll();
	...
	public function actionMenciones() {
		...
		$picsMenciones = ViewPicsCalificadas::model ()->findAll ( array (
				"condition" => "b_mencion=1 AND id_contest= 6" 
		) );
	...
	private function createCriteriaCompetitors($string = '', $feedback = '', $sort = '', $competitors=array(), $mencion=0) {
		...
		$criteria->condition = " U.id_usuario IN (select SUC.id_usuario FROM 2gom_con_rel_users_contest SUC) AND UC.id_contest = 6";
